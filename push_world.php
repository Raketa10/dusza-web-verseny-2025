<?php
    ...
    if(isset($_SESSION["user_id"])){
        $statement = $connection->prepare("SELECT * FROM damareen_worlds WHERE world_id = ? AND user_id = ?");
        ...
        if ($result->num_rows > 0) {
            $statement = $connection->prepare("UPDATE damareen_worlds SET name = ?, cards_json = ?, casemates_json = ?, collection_json = ? WHERE world_id = ? AND user_id = ?");
            ...
            $statement = $connection->prepare("INSERT INTO damareen_worlds (world_id, name, cards_json, casemates_json, collection_json, user_id) VALUES(?, ?, ?, ?, ?, ?)");
